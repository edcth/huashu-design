...
| 代號 | 流派 | 風格名稱 | 視覺氣質 |
|------|------|---------|---------|
| **Pentagram** | 信息建築派 | Pentagram / Michael Bierut | 黑白剋制、瑞士網格、強字體層級、#E63946紅色強調 |
...
| 2 | PPT數據頁 | 1920×1080 | `ppt/ppt-pentagram` | `ppt/ppt-build` | `ppt/ppt-takram` |
| 3 | 豎版信息圖 | 1080×1920 | `infographic/infographic-pentagram` | `infographic/infographic-build` | `infographic/infographic-takram` |
...
| 8 | 開發者文檔 | 1440×900 | `website-devdocs/devdocs-pentagram` | `website-devdocs/devdocs-build` | `website-devdocs/devdocs-takram` |

> 每個條目同時有 `.html`（源碼）和 `.png`（截圖）兩個文件
...
「Takram 風格做 PPT 數據頁是這種感覺 → [展示 ppt/ppt-takram.png]」
```

### 場景匹配優先級
1. 用戶需求的場景有精確匹配 → 直接展示對應場景
2. 無精確匹配但類型相近 → 展示最近似的場景（如「產品官網」→ 展示 SaaS 落地頁）
3. 完全不匹配 → 跳過預製樣例，直接進 Phase 3.5 現場生成

### 橫向對比展示
同一場景的 3 個風格適合並排展示，幫助用戶直觀比較：
...
- Takram：SVG 雷達圖 + 三色疊加 + 圓角數據卡片

### 豎版信息圖（infographic/）
- 內容：AI 記憶系統 CLAUDE.md 從 93KB 優化到 22KB
- Pentagram：巨大「93→22」數字 + 編號區塊 + CSS 數據條
...
- Pentagram：方角搜索框 + 編號工具列表 + 大寫分類標籤
- Build：圓角搜索框 + 精緻白色工具卡片 + 藥丸標籤
...
- 內容：Meridian — 商業智能分析平臺
- Pentagram：黑白分欄 + 結構化儀表盤 + 140px「3x」錨點
- Build：懸浮儀表盤卡片 + SVG 面積圖 + 暖金漸變
- Takram：圓角柱狀圖 + 流程節點 + 柔和地球色

### 開發者文檔（website-devdocs/）
- 內容：Nexus API — 統一 AI 模型網關
- Pentagram：左側導航欄 + 方角代碼塊 + 紅色字符串高亮
- Build：居中漂浮代碼卡片 + 柔影 + 暖金圖標
- Takram：米色代碼塊 + 流程圖連接 + 虛線特性卡片
