...
| `sfx/ui/click-soft.mp3` | 0.5s | 柔和 UI click（次要按鈕/鏈接） | soft gentle button click, mid pitched |
...
| `sfx/ui/tap-finger.mp3` | 0.5s | 移動端 tap（iOS 界面） | finger tap on touchscreen, muted thud |
...
| `sfx/container/modal-open.mp3` | 0.6s | 模態框打開 | modal popping open, whoosh + thud |
...
| `sfx/feedback/notification-pop.mp3` | 0.6s | 消息彈出（toast、通知） | notification bloop, ios message alert |
...
| 文件 | 時長 | 用途 | Prompt 要點 |
|---|---|---|---|
| `sfx/progress/loading-tick.mp3` | 0.5s | 加載計時（進度條節拍） | soft short pulse, minimal ambient |
...
| `sfx/terminal/cursor-blink.mp3` | 0.5s | 光標閃爍 | subtle soft digital pulse, rhythmic |
...
```
循環元素：`cursor-blink` 作為 idle 時的環境音。

### 🃏 卡片選擇流程
```
hover-subtle (0.5s, UI懸停) → click-soft (0.5s, 點擊) → card-snap (0.5s, 定位)
```
...
### 📊 數據可視化/儀表盤
```
loading-tick (0.5s, 節拍) × N → complete-done (0.8s, 數據到位) → achievement (1.5s, 驚豔落點)
```
...
### 音量建議（來自 apple-gallery-showcase.md 音頻雙軌制）
- **SFX 主軌**：`1.0`（不做衰減）
...
### 選型決策樹
1. **有 tactile 動作**（打字/點擊/滑動）→ `keyboard/` or `ui/`
...
## 詳見
- 音頻雙軌制與 ffmpeg 拼接：`apple-gallery-showcase.md`
