...
> 靈感來源：Claude Design 官網 hero 視頻 + 蘋果產品頁「作品牆」式陳列
> 實戰出處：huashu-design 發佈 hero v5
> 適用場景：**產品發佈 hero 動畫、skill 能力演示、作品集展示**——任何需要把「多件高質量產出」同時展陳並引導觀眾注意力的場景
...
**適合**：
- 有10張以上真實產出要同屏展示（PPT、App、網頁、信息圖）
- 觀眾是專業受眾（開發者、設計師、產品經理），對「質感」敏感
- 希望傳遞的氣質是「剋制、展覽式、高級、有空間感」
- 需要焦點和全局同時存在（看細節但不失整體）
...
3. **三字體棧**（serif英+serif中+sans+mono）營造「出版物」而非「互聯網產品」的氣質
...
**反面教材**：不要貼邊瓷磚（無padding無border無shadow）——那是信息圖密度表達，不是展覽。
...
用正弦波 + 線性漂移組合，避免 marquee 那種「有起點有終點」的循環感：
...
**關鍵設計**：focus overlay 是一個**平面元素**（不傾斜），浮在傾斜畫布之上。被選中的 slide 從瓦片位置（約400×225）縮放到屏幕中央（960×540），背景畫布不傾斜變化但**變暗到 45%**：
...
.brand-wordmark .accent {
  color: var(--accent);
  font-weight: 500;           /* accent字符反而細一點，視覺差 */
}
```
...
| Pan 感覺「卡頓」 | 用了 setTimeout 或 CSS keyframes 循環 | 用 rAF + sin/cos 連續函數 |
| Focus 時字看不清 | 複用了 gallery 瓦片的低分圖 | 獨立 overlay + 原圖 src 直連 |
| 背景太空 | 純色 `#F5F5F7` | 疊加 SVG fractalNoise 0.5 opacity |
| 字體太"互聯網" | 只有 Inter | 加 Serif（中英各一）+ mono 三棧 |

---

## 引用

- 完整實現樣本：`/Users/alchain/Documents/寫作/01-公眾號寫作/項目/2026.04-huashu-design發佈/配圖/hero-animation-v5.html`
- 原始靈感：claude.ai/design hero 視頻
- 參考審美：Apple 產品頁、Dribbble shot 集合頁

遇到「多件高質量產出要陳列」的動畫需求，直接從此文件 copy 骨架，換內容 + 調 timing 即可。
