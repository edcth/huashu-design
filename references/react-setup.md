...
**不要**用`react@18`或`react@latest`這種unpinned版本——會出現版本漂移/緩存問題。
...
項目名/
├── index.html               # 主HTML
├── components.jsx           # 組件文件（type="text/babel"加載）
├── data.js                  # 數據文件
└── styles.css               # 額外CSS（可選）
```
...
**或者用inline styles**（小組件推薦）：
```jsx
<div style={{ padding: 16, background: '#111' }}>...</div>
```

這條是**非協商**的。每次寫`const styles = {...}`都必須replace成specific命名，否則多組件加載時全棧報錯。

### 規矩2：Scope 不共享，需手動export

**關鍵認知**：每個`<script type="text/babel">`被Babel獨立編譯，它們之間**scope不通**。`components.jsx`裡定義的`Terminal`組件，在`pages.jsx`裡**默認是undefined**。

**解決方式**：在每個組件文件末尾，把要共享的組件/工具export到`window`：
...
如果你的 HTML 原型需要調用 LLM 做 demo（比如做個聊天 interface），兩個選項：
...
需要API key，用戶必須在HTML裡填入自己的key才能跑。**永遠不要把key硬編碼在HTML裡**。

```html
<input id="api-key" placeholder="粘貼你的Anthropic API key" />
<script>
window.claude = {
  async complete(prompt) {
...
**注意**：瀏覽器直接調Anthropic API會遇到CORS問題。如果用戶給你的預覽環境不支持CORS bypass，這條路不通。這時候用選項A mock，或者告訴用戶需要一個proxy後端。

### 選項 C：用 agent 側的 LLM 能力生成 mock 數據

如果只是本地演示用，可以在當前 agent 會話裡臨時調用該 agent 的 LLM 能力（或用戶裝的 multi-model 類 skill）先生成 mock 響應數據，再硬編碼寫進 HTML。這樣 HTML 運行時完全不依賴任何 API。
...
→ 你在一個文件裡定義了`const styles`，另一個文件覆蓋了。給每個改成specific命名。

**`Terminal is not defined`**
→ 跨文件引用時scope不通。在定義Terminal的文件末尾加`Object.assign(window, {Terminal})`。

**整個頁面白屏，控制檯沒錯誤**
→ 多半是JSX語法錯誤但Babel沒報在控制檯。把`babel.min.js`臨時換成`babel.js`非壓縮版，錯誤信息更清晰。

**ReactDOM.createRoot is not a function**
→ 版本不對。確認用了react-dom@18.3.1（而不是17或其他）。

**`Objects are not valid as a React child`**
→ 你渲染了一個對象而不是JSX/字符串。通常是`{someObj}`寫成了`{someObj.name}`。
...
**每個文件末尾**都要`Object.assign(window, {...})`導出要共享的東西。
