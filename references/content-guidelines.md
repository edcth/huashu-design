...
AI設計裡最容易掉進去的陷阱。這是一份「不做什麼」的清單，比「做什麼」更重要——因為AI slop是默認值，你不主動避免就會發生。
...
沒圖標用真icon庫（Lucide/Heroicons/Phosphor），或者用placeholder。

**❌ SVG 畫 imagery**
不要試圖用SVG畫：人物、場景、設備、物品、抽象藝術。AI畫的SVG imagery一眼就是AI味，幼稚且廉價。**一個灰色矩形+"插畫位 1200×800"的文字標籤，比一個拙劣的SVG hero illustration強100倍**。
...
**❌ 過多iconography**
不是每個標題/feature/section都需要icon。濫用icon會讓界面像toy。Less is more。

**❌ "Data slop"**
編造的stats裝飾：
- "10,000+ happy customers" （你都不知道有沒有）
- "99.9% uptime" （沒有真數據就別寫）
- 用圖標+數字+詞組成的裝飾"metric cards"
- Mock table裡的假數據裝點得花裡胡哨

如果沒真數據，留placeholder或問用戶要。

**❌ "Quote slop"**
編造的用戶評價、名人名言裝飾頁面。留placeholder問用戶要真quote。
...
- 開源字體站（Fraunces的兄弟字體、Adobe Fonts）
- 不要憑空發明字體名
...
3. 完全從零 → 選一個known的配色系統（Radix Colors / Tailwind默認palette / Anthropic brand），不要自己調
...
**❌ Bento grid 過度氾濫**
每個AI生成的landing page都想搞bento。除非你的信息structure確實適合bento，否則用其他layout。
...
- 這個stats/quote/feature有真數據支持嗎？沒有就不要憑空寫。

「One thousand no's for every yes」。

### 2. Ask before adding material

你覺得多加一段/一頁/一個section會更好？先問用戶，不要單方面加。
...
探索完design context後，**先口頭說出你要用的系統**，讓用戶確認：
...
- 圖像：hero用full-bleed照片，feature section用placeholder等你提供
- 最多用2種背景色，避免雜亂

確認這個方向我就開始做。
```

用戶確認後再動手。這個check-in能避免"做完一半發現方向錯"。
...
- 想加一段quote裝飾？→ 先問用戶有沒有真quote
- 想加一排icon features？→ 先問要不要icon，可能不需要
- 用Inter？→ 換一個更有特點的
- 用紫色漸變？→ 換一個有根據的配色

**當你覺得"加一下會更好看"的時候——那通常是AI slop的徵兆**。先做最簡的版本，只在用戶要求時加。
