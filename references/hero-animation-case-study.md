...
通用 SaaS feature 動畫、產品發佈會、skill 推廣、系列作品集展示——只要素材數量夠、風格一致，這套結構幾乎都能出效果。
...
**第二拍 · Multi-Focus（~8s，4 次循環）**：鏡頭在慢速 pan 的同時，4 次把背景 dim + desaturate，把某一張卡單獨放大到屏幕中央——觀眾從"量的衝擊"切換到"質的凝視"，每次 1.7s 節奏穩定。
...
1. **素材規模 ≥ 20 張，最好 30+**
   少於 20 張 Ripple 會顯得"空"——48 格里每格都在動才有密度感。v9 用了 48 格 × 32 張圖（循環填充）。

2. **素材視覺風格一致**
   全是 16:9 slide 預覽 / 全是 app 截圖 / 全是封面設計——長寬比、色調、版式得像是"一套"。混搭會讓 Gallery 看起來像剪貼板。

3. **素材單獨放大後仍有可讀信息**
   Focus 是把某張卡放大到 960px 寬，如果原圖放大後糊了或信息稀薄，Focus 這一拍就廢了。反向驗證：能不能從 48 張裡挑出 4 張作為"最有代表性"的？挑不出來就說明素材質量不齊。
...
- 正弦波 + 線性 drift 雙層運動——不是純循環，每個時刻位置都不同
...
**為什麼**：溫暖底色在 GIF 壓縮後依然有"呼吸感"，不像純白會顯得"屏幕感"。赤陶橙作為唯一 accent 貫穿 terminal prompt、dir-card 選中、cursor、brand hyphen、focus ring——所有視覺錨點都被這一個色串起來。
...
用 `sin(i × 1.7) + cos(i × 0.73)` 確定性算法給每張卡分配 near/mid/far 三檔 shadow——**視覺上有"三維堆疊"感，但每幀 transform 完全不變，GPU 消耗 0**。
...
**限制**：必須用支持 variable font 的字體（Inter/Roboto Flex/Recursive 等）。普通靜態字體只能擬態（切換幾個固定 weight 有跳變）。
...
**❌ 數據驅動內容**：觀眾要讀數字，Gallery 的快速節奏不給時間讀。改用「數據圖表 + 逐項 reveal」。
...
三步都 yes，直接 fork v6 HTML，改 `SLIDE_FILES` 數組和時間軸就能複用。調色板改 `--bg / --accent / --ink`，整體換皮不換骨。
...
- 動畫導出流水線：[references/video-export.md](video-export.md)
- 音頻配置（BGM + SFX 雙軌）：[references/audio-design-rules.md](audio-design-rules.md)
- Apple 畫廊風格的橫向參考：[references/apple-gallery-showcase.md](apple-gallery-showcase.md)
- 源 HTML（v6 + 音頻集成版）：`www.huasheng.ai/huashu-design-hero/index.html`
