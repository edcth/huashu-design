# 可編輯 PPTX 導出：HTML 硬約束 + 尺寸決策 + 常見錯誤

本文檔講的是**用 `scripts/html2pptx.js` + `pptxgenjs` 把 HTML 逐元素翻譯成真·可編輯 PowerPoint 文本框**的路徑，也是 `export_deck_pptx.mjs` 唯一支持的路徑。

> **核心前提**：要走這條路，HTML 必須從第一行就按下面 4 條約束寫。**不是寫完再轉**——事後補救會觸發 2-3 小時返工（2026-04-20 期權私董會項目實測踩坑）。
>
> 視覺自由度優先的場景（動畫 / web component / CSS 漸變 / 複雜 SVG）請改走 PDF 路徑（`export_deck_pdf.mjs` / `export_deck_stage_pdf.mjs`），**不要**指望 pptx 導出能兼得視覺保真和可編輯——這是 PPTX 文件格式本身的物理約束（見文末「為什麼 4 條約束不是 Bug 而是物理約束」）。
...
|---|---|---|---|
| **`960pt × 540pt`** | **13.333″ × 7.5″** | **pptxgenjs `LAYOUT_WIDE`** | ✅ **默認推薦**（現代 PowerPoint 16:9 標配） |
| `720pt × 405pt` | 10″ × 5.625″ | 自定義 | 僅當用戶指定「老版 PowerPoint Widescreen」模板時 |
| `1920px × 1080px` | 20″ × 11.25″ | 自定義 | ❌ 非標尺寸，投影后字體顯得異常小 |

**別把 HTML 尺寸當分辨率想。** PPTX 是矢量文檔，body 尺寸決定的是**物理尺寸**不是清晰度。超大 body（20″×11.25″）不會讓文字更清晰——只會讓字號 pt 相對畫布變小，投影/打印時反而更難看。
...
`html2pptx.js` 把 HTML 的 DOM 逐元素翻譯成 PowerPoint 對象。PowerPoint 的格式約束投射到 HTML 上 = 下面 4 條規則。
...
<div class="body"><p>新用戶是主要驅動力</p></div>
```

**為什麼**：PowerPoint 文本必須存在 text frame 裡，text frame 對應 HTML 的段落級元素（p/h*/li）。裸 `<div>` 在 PPTX 裡沒有對應的文本容器。

**也不能用 `<span>` 承載主文字**——span 是行內元素，沒法獨立對齊成文本框。span 只能**夾在 p/h\* 裡**做局部樣式（加粗、換色）。

### 規則 2：不支持 CSS 漸變 — 只能用純色
...
**為什麼**：PowerPoint 的 shape fill 只支持 solid/gradient-fill 兩種，但 pptxgenjs 的 `fill: { color: ... }` 只映射 solid。漸變走 PowerPoint 原生 gradient 需要另寫結構，目前工具鏈不支持。
...
**為什麼**：PowerPoint 裡 shape（方塊/圓角矩形）和 text frame 是兩個對象。HTML 的 `<p>` 只翻譯成 text frame，背景/邊框/陰影屬於 shape——必須在**包裹 text 的 div** 上寫。
...
每張 slide 一個獨立 HTML 文件，彼此作用域隔離（避開單文件 deck 的 CSS 汙染）。
...
我的Deck/
├── slides/
│   ├── 01-cover.html    # 每個文件都是完整 960×540pt HTML
│   ├── 02-agenda.html
│   └── ...
...
### Step 2：寫 build.js 調用 `html2pptx.js`
...
| 只是演講用 / 發存檔，不再改 | `export_deck_pdf.mjs`（多文件）或 `export_deck_stage_pdf.mjs`（單文件 deck-stage），出矢量 PDF |
...
## Fallback：已有視覺稿但用戶堅持要 editable PPTX

偶爾會遇到這個場景：你/用戶已經寫好一份視覺驅動的 HTML（漸變、web component、複雜 SVG 都用上了），本來出 PDF 最合適，但用戶明確說「不行，必須是可編輯的 PPTX」。
...
> - B. **以視覺稿為藍本，重寫一版 editable HTML**（保留色彩/佈局/文案的設計決策，但按 4 條硬約束重新組織 HTML 結構，**犧牲**漸變、web component、複雜 SVG 等視覺能力）→ 再導出 editable PPTX
>
> 你選哪個？」

不要把 B 方案說得雲淡風輕——明確告知**會丟失什麼**。讓用戶做取捨。

### Step 2 · 如果用戶選 B：AI 主動改寫，不要求用戶自己寫

這裡的 doctrine 是：**用戶給的是設計意圖，你負責翻譯成合規實現**。不是讓用戶去學 4 條硬約束然後自己重寫。

改寫時的遵循原則：
- **保留**：色彩系統（主色/輔色/中性色）、信息層級（標題/副標題/正文/註解）、核心文案、layout 骨架（上中下 / 左右分欄 / 網格）、頁面節奏
- **降級**：CSS 漸變 → 純色或 flex 分段、web component → 段落級 HTML、複雜 SVG → 簡化的 `<img>` 或純色幾何、陰影 → 刪除或降為極弱、自定義字體 → 向系統字體靠齊
- **重寫**：裸文字 → 包進 `<p>` / `<h*>`、`background-image` → `<img>` 標籤、`<p>` 上的背景邊框 → 外層 div 承載

### Step 3 · 產出對照清單（透明交付）

改寫完成後給用戶一份 before/after 對照，讓他知道哪些視覺細節被簡化了：

```
原設計 → editable 版調整
- 標題區紫色漸變 → 主色 #5B3DE8 純色背景
- 數據卡片陰影 → 刪除（改為 2pt 描邊區分）
...
- 雙格式交付給用戶：視覺稿的 PDF + 可編輯的 PPTX，各司其職

### 什麼情況下直接拒絕 B 方案

個別場景下改寫代價過高，應該勸用戶放棄 editable PPTX：
- HTML 核心價值是動畫或交互（改寫後只剩靜態首幀，信息量損失 50%+）
- 頁數 > 30，改寫成本超過 2 小時
- 視覺設計深度依賴精確 SVG / 自定義 filter（改寫後和原圖幾乎無關）

此時告訴用戶：「這個 deck 改寫代價過高，建議出 PDF 而不是 PPTX。如果接收方確實要 pptx 格式，就接受視覺會大幅樸素化——要不要換成 PDF？」

---

## 為什麼 4 條約束不是 Bug 而是物理約束

這 4 條不是 `html2pptx.js` 作者偷懶——它們是 **PowerPoint 文件格式（OOXML）本身的約束**投射到 HTML 上的結果：

- PPTX 裡文字必須在 text frame（`<a:txBody>`），對應段落級 HTML 元素
- PPTX 的 shape 和 text frame 是兩個對象，無法在同一 element 上同時畫背景和寫文字
- PPTX 的 shape fill 對 gradient 支持有限（僅某些 preset gradients，不支持 CSS 任意角度漸變）
- PPTX 的 picture 對象必須引用真實圖片文件，不是 CSS 屬性
